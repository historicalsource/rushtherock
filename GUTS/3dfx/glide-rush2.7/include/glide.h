...
**
** $Header: /engr/devel/SST1/GLIDE/DOS/SRC/GLIDE.H 105   6/16/96 5:05p Jdt $
** $Log: /engr/devel/SST1/GLIDE/DOS/SRC/GLIDE.H $
 * 
 * 105   6/16/96 5:05p Jdt
 * Fixed grTexMipMapMode and grTexSource signatures.
 * 
 * 104   6/15/96 5:17p Dow
 * Removed grAADrawEdges
 * 
 * 103   6/14/96 10:04a Dow
 * Added grGlideShamelessPlug(FxBool on);
 * 
 * 102   6/13/96 2:21p Dow
 * Got rid of a const in a declaration that was giving C++ headaches.
 * 
 * 101   6/12/96 5:27p Dow
 * Removed grSstInVerticalBlank
 * Added grSstVRetraceOn
 * 
 * 99    6/11/96 7:39p Dow
 * Added pixel performance metrics routines, removed old ones.
 * 
 * 98    6/10/96 12:08p Dow
 * Added grAADrawPolygonVertexList
 * Added grAADrawEdges
 * Removed guAADrawTriangleWithClip (it's moved to glideutl.h)
 * 
 * 97    6/10/96 12:37a Jdt
 * Changed declaration of GR_NULL_MIPMAP_HANDLE to fix warnings
 * Added paletized texture formats and renumbered reserved fields
 * Added constants for GrTexTable_t
 * Added constants for GrNCCTable_t
 * Added constants for GrTexBaseRange_t
 * Resized GLIDE_STATE_PAD_SIZE for new state
 * Added palette entry to Gu3dfInfo
 * Created GuTexTable union
 * Created GuTexPalette structure
 * Created GrTexInfo structure
 * Removed const modifier of GuNccTable type in GrMipMapInfo structure,
 * this can't be initialized without casting and breaks
 * C++
 ...
 *    grTexMipMapMode
 *    grTexGetCurrentMipMap->guTexGetCurrentMipMap
 * 
 * 96    6/08/96 4:16p Dow
 *      Added conditional definition of FX_BIT
 *      Reordered GrVertex structure to reflect order the registers appear in
 * the hardware
 *      Changed hints
 *      s/grTexMemQueryAvail/guTexMemqueryAvail/g
 * 
 * 95    6/03/96 1:17p Dow
 ...
 * s/grTexMemQueryAvail/guTexMemQueryAvail/g
 * 
 * 94    5/28/96 4:47p Paik
 * Move GrErrorCallback to private header file
 * 
 * 93    5/23/96 12:21p Dow
 * Changed around grAADrawPoints & DrawLines
 * made grAA{Begin,End}{Points,Lines} internal routines.
 * 
 * 92    5/22/96 6:25p Paik
 * Fix stupid GrErrorCallback warnings
 * 
 * 91    5/22/96 10:07a Dow
 * Adjustments for hardware config info structure.
 * 
 * 90    5/21/96 2:21p Dow
 * 
 * 89    5/20/96 11:55a Dow
 * Fixed up NCC texture stuff.
 * 
 * 88    5/14/96 8:15a Dow
 * Added GrHints() and associated type
 * 
 * 87    5/09/96 7:04a Paik
 * Glide DLL support
 * 
 * 86    5/08/96 3:07p Paik
 * Clean up of build warnings
 * 
 * 85    4/28/96 12:37p Sellers
 * 
 * 84    4/28/96 12:35p Sellers
 * added 856x480 resolution
 * 
 * 83    4/25/96 7:12p Jeffrey
 * Fixed typographical error in #define GR_BLEND_RESERVED_E.
 * 
 * 82    4/25/96 5:03p Sellers
 * added 960x720 resolution
 * 
 * 81    3/25/96 7:58p Paik
 * Move fxglide.h out of glide.h
 * 
 * 80    3/03/96 5:38a Paik
 * Remove some redundant or obsolete prototypes and reorder a little bit.
 * 
 * 79    3/01/96 2:17p Paik
 * Update GLIDE_STATE_PAD
 * 
 * 78    3/01/96 2:37a Paik
 * Increase glide state pad
 * 
 * 77    2/29/96 4:21p Paik
 * Prepare to slim down the state
 * 
 * 76    2/28/96 4:49p Dow
 * added grvertex indeces
 * 
 * 75    2/21/96 9:59a Dow
 * Fixed bogus define in GR_COMBINE_LOCAL_DEPTH
 * 
 * 74    2/21/96 9:51a Dow
 * Changed enums to defines.  Typedefs changed from enums to FxU32s
 * 
 * 73    2/20/96 2:34p Paik
 * DJGPP hack
 * 
 * 72    2/13/96 3:00p Dow
 * got rid of grDrawBuffer_t, removed spurious constant values for
 * grBuffer_t elements
 * 
 * 71    2/12/96 3:20p Dow
 * :wq
 * 
 * 70    2/12/96 8:34a Dow
 * GR_STATE_PAD
 * 
 * 69    2/11/96 4:21p Dow
 *  num_tmu is now 3, added grSstIdle()  (this is faster than a
 * while(grSstIsBusy()) loop)
 * 
 * 68    2/09/96 4:57p Dow
 * GrState size stuff
 * 
 * 65    2/06/96 3:13p Dow
 * Added 640x400
 * 
 * 64    2/06/96 6:34a Dow
 * Changed state pad size
 * 
 * 63    2/05/96 7:10p Sellers
 * Added GR_RESOLUTION_640x200 and GR_RESOLUTION_640x350
 * 
 * 62    2/02/96 11:54p Paik
 * Alpha saturate alpha blend function had wrong enum value for hardware.
 * Added prefog color alpha blend function.
 * Change size of GLIDE_STATE_PAD for new texture combine.
 * Add GR_TEXTURECOMBINE_DETAIL_OTHER to complement
 * GR_TEXTURECOMBINE_DETAIL.
 * Add prototype for grTexCombine.
 * 
 * 65    1/31/96 11:21p Paik
 * Add prototype for grTexCombine
 * 
 * 64    1/31/96 6:04p Paik
 * Change size of GLIDE_STATE_PAD for new texture combine,
 * Add GR_TEXTURECOMBINE_DETAIL_OTHER to complement
 * GR_TEXTURECOMBINE_DETAIL.
 * 
 * 63    1/30/96 10:20p Paik
 * Alpha saturate alpha blend function had wrong enum value for hardware.
 * 
 * 62    1/30/96 5:54p Paik
 * 
 * 61    1/24/96 3:48a Paik
 * Add grDrawBuffer
 * 
 * 60    1/15/96 6:29p Dow
 * Hardware Bringup mods
 * 
 * 59    1/14/96 5:41p Dow
 * size change for MSVC
 * 
 * 57    1/14/96 1:53p Dow
 * GLIDE_STATE_PAD -> GLIDE_STATE_PAD_SIZE
 * 
 * 55    1/14/96 9:27a Dow
 * cosmetic mostly, but also changed size of GrState
 * 
 * 54    1/13/96 2:43p Dow
 * Random fixes, and emacs helping stuff
 * 
 * 53    1/12/96 4:16p Dow
 * consistency for types 
 * 
 * 52    1/12/96 4:13p Dow
 * updated sizeo of GrState
 * 
 * 51    1/12/96 2:28p Paik
 ...
 * 
 * 50    1/12/96 2:00p Dow
 * hw bringup and GrChipID_t stuff
 * 
 * 46    1/11/96 2:26a Dow
 ...
 * modes.
 * 
 * 45    1/11/96 12:56a Dow
 * GLIDE_NUM_TMU back to 2
 * 
 * 44    1/10/96 11:12p Dow
 * more 1 & 3 tmu, virtual tmu removal fixes
 * 
 * 43    1/09/96 9:39a Dow
...
#define GLIDE_VERSION	203	/* actually 2.0b3 */
...
*/
#ifndef MAX_NUM_SST
#define MAX_NUM_SST            2
#endif
#ifndef MAX_MIPMAPS_PER_SST
...
#define GR_TEXBASE_0        0x0
#define GR_TEXBASE_1        0x1
#define GR_TEXBASE_2        0x2
#define GR_TEXBASE_3_TO_8   0x3

#define GLIDE_STATE_PAD_SIZE 304
#ifdef GLIDE_LIB
...
{
  GrTextureFormat_t
    format;                     /* format of the texture table */
  GrMipMapMode_t
    mipmap_mode;                /* mip map mode for this texture */
  GrTextureFilterMode_t
    magfilter_mode;             /* filtering to be used when magnified */
  GrTextureFilterMode_t
    minfilter_mode;             /* filtering to be used with minified  */
  GrTextureClampMode_t
    s_clamp_mode;               /* how this texture should be clamped in s */
  GrTextureClampMode_t
    t_clamp_mode;               /* how this texture should be clamped in t */
  FxU32
    tLOD;                       /* Register value for tLOD register when
                                   this texture is selected as current
                                   This value is computed at run-time during
                                   the call to guTexAllocateMemory  */
  FxU32
    tTextureMode;               /* Register value for tTextureMode register
                                   not including non-texture specific bits */

  FxU32
    lod_bias;                   /* LOD bias of the mip map in preshifted 4.2*/
  GrLOD_t
    lod_min, lod_max;           /* largest and smallest levels of detail  */
  int
    tmu;                        /* tmu on which this texture resides */
  FxU32
    odd_even_mask;              /* mask specifying levels on this tmu  */
  FxU32
    tmu_base_address;           /* base addr (in TMU mem) of this texture */
  FxBool
    trilinear;                  /* should we blend by lod? */

  GrAspectRatio_t
    aspect_ratio;               /* aspect ratio of the mip map.  */


  void
    *data;                      /* actual texture data  */
  GuNccTable
    ncc_table;                  /* NCC compression table (optional) */

  int
    sst;                        /* SST where this texture map was stored  */

  FxBool
    valid;                      /* set when this table entry is allocated*/
  int
    width, height;
} GrMipMapInfo;
...
typedef struct GrVoodooConfig_St {
  int
    fbRam;                      /* 1, 2, or 4 MB */
  int
    fbiRev;                     /* Rev of Pixelfx chip */
  int
    nTexelfx;                   /* How many texelFX chips are there? */
  FxBool
    sliDetect;                  /* Is it a scan-line interleaved board? */
  GrTMUConfig_t
    tmuConfig[GLIDE_NUM_TMU];   /* Configuration of the Texelfx chips */
} GrVoodooConfig_t;
...
typedef struct GrSstConfig_St {
  GrSstType
    type;                       /* Which hardware is it? */
  union SstBoard_u {
...
typedef struct {
  int
    num_sst;                    /* # of HW units in the system */
  GrSstConfig_t
...
typedef struct GrSstPerfStats_s {
  FxU32
    pixelsIn;                   /* # pixels processed */
  FxU32
    chromaFail;                 /* # pixels not drawn due to chroma
                                   key */ 
  FxU32
    zFuncFail;                  /* # pixels not drawn due to Z
                                   comparison */
  FxU32
    aFuncFail;                  /* # pixels not drawn due to alpha
                                   comparison */
  FxU32
    pixelsOut;                  /* # pixels drawn */
} GrSstPerfStats_t;


typedef struct {
  float
    sow;                        /* s texture ordinate (s over w) */
  float
    tow;                        /* t texture ordinate (t over w) */  
  float
    oow;                        /* 1/w (used mipmapping - really 0xfff/w) */
}  GrTmuVertex;
...
#define GR_STWHINT_W_DIFF_FBI   FX_BIT(0)
#define GR_STWHINT_W_DIFF_TMU0  FX_BIT(1)
#define GR_STWHINT_ST_DIFF_TMU0 FX_BIT(2)
#define GR_STWHINT_W_DIFF_TMU1  FX_BIT(3)
#define GR_STWHINT_ST_DIFF_TMU1 FX_BIT(4)
#define GR_STWHINT_W_DIFF_TMU2  FX_BIT(5)
#define GR_STWHINT_ST_DIFF_TMU2 FX_BIT(6)
...
*/
FX_ENTRY void
FX_CALL grDrawLine( const GrVertex *v1, const GrVertex *v2 );

FX_ENTRY void
FX_CALL grDrawPlanarPolygon( int nverts, const int ilist[], const GrVertex vlist[] );

FX_ENTRY void
FX_CALL grDrawPoint( const GrVertex *pt );

FX_ENTRY void
FX_CALL grDrawPolygon( int nverts, const int ilist[], const GrVertex vlist[] );

FX_ENTRY void
FX_CALL grDrawPolygonVertexList( int nverts, const GrVertex vlist[] );

FX_ENTRY void
FX_CALL grDrawTriangle( const GrVertex *a, const GrVertex *b, const GrVertex *c );

FX_ENTRY void
FX_CALL _grVerticesToSST( const GrVertex *a, const GrVertex *b, const GrVertex *c );

FX_ENTRY void
FX_CALL agcDrawTriBlend( const GrVertex *a, const GrVertex *b, const GrVertex *c );

FX_ENTRY void
FX_CALL agcBufferSwap(int interval);

/*
** buffer management
*/
FX_ENTRY void
FX_CALL grBufferClear( GrColor_t color, GrAlpha_t alpha, FxU16 depth );

FX_ENTRY int
FX_CALL grBufferNumPending( void );

FX_ENTRY void
FX_CALL grBufferSwap( int swap_interval );

FX_ENTRY void
FX_CALL grRenderBuffer( GrBuffer_t buffer );
...
FX_ENTRY GrLfbWriteMode_t FX_CALL 
...
#  define GRLFBWRITE16( a, b ) ( grSstWriteAbsolute16( ( FxU32 ) (a), (b) ) )
#  define GRLFBWRITE32( a, b ) ( grSstWriteAbsolute32( ( FxU32 ) (a), (b) ) )
#else
#  define GRLFBREAD32( a )     ( *(a) )
#  define GRLFBWRITE32( a, b ) ( *(a) = (b) )
#endif
