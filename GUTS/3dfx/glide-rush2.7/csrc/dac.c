...
**
*/
 FxU32  sst1InitDacRd(FxU32 *sstbase, FxU32 addr)
{
    volatile Sstregs *sst = (Sstregs *) sstbase;
    FxU32 retVal;
    int helper = (GETENV(("SST_DEBUGDAC"))) ? 1 : 0;

    sst->dacData = ((addr) << SST_DACDATA_ADDR_SHIFT) | SST_DACDATA_RD;
    sst1InitIdleFBINoNOP(sstbase);
    retVal = sst->fbiInit2 & SST_DACDATA_DATA;
    if(helper)
...
**
*/
 void  sst1InitDacWr(FxU32 *sstbase, FxU32 addr, FxU32 data)
{
    volatile Sstregs *sst = (Sstregs *) sstbase;
    int helper = (GETENV(("SST_DEBUGDAC"))) ? 1 : 0;
...
**
*/
 FxBool  sst1InitDacDetect(FxU32 * sstbase)
{
    FxU32 n;
    FxU32 fbiInit1_save;
    FxU32 fbiInit2_save;
    ...
    fbiInit1_save = sst->fbiInit1;
    fbiInit2_save = sst->fbiInit2;
    ...
    sst1InitIdleFBINoNOP(sstbase);
    sst->fbiInit1 = fbiInit1_save;
    sst->fbiInit2 = fbiInit2_save;
    sst1InitIdleFBINoNOP(sstbase);
    return(retVal);
}
 ...
 FxBool  sst1InitCalcGrxClk(FxU32 *sstbase)
{
    ...
    if(GETENV(("SST_GRXCLK"))) {
        ...
        if(sst1CurrentBoard->fbiBoardID == 0x0) {
            /* Obsidian GE Fab */
            clkFreq = 40 + (((sst1CurrentBoard->tmuConfig >> 3) & 0x3) << 2) +
...
# if USE_MHZ
	    if (!grPixelClockFreq) {
		if (clkFreq > USE_MHZ) clkFreq = USE_MHZ;
	    } else {
		clkFreq = grPixelClockFreq;
	    }
# else
	    if (clkFreq > 45) clkFreq = 45;
# endif
#endif
        }
    }
    ...
    *clkTiming)
{
    ...
    /* ulate proper N and M parameters which yield the lowest error */
...
**
*/
 FxBool  sst1InitDacIndexedEnable(FxU32 *sstbase,
  FxU32 Enable)
{
