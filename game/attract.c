 ...
 *  ======================================================================
 *  $Author: gray $                       $Date $
 *  $Revision: 3.172 $                     $Locker:  $
 *  ======================================================================
 *  Change Log:
 *
 *  $Log: attract.c,v $
 * Revision 3.172  1997/10/16  20:17:25  gray
 * version 1.00!
 *
 * Revision 3.171  1997/10/16  03:14:26  gray
 * version 0.24.
 *
 * Revision 3.170  1997/10/16  02:52:39  grjost
 * Hacked in continue mode
 *
 * Revision 3.169  1997/10/14  21:06:50  gray
 * version 0.23.
 *
 * Revision 3.168  1997/10/13  21:58:03  gray
 * version 0.22 - skip aborted IRQTIME reset version 0.21
 *
 * Revision 3.167  1997/10/13  03:55:38  gray
 * Reenabled flashing press-start for new credit screens.
 *
 * Revision 3.166  1997/10/11  01:47:03  gray
 * version 0.19.
 *
 * Revision 3.165  1997/10/09  19:33:13  hightower
 * Synchonized blinks
 *
 * Revision 3.164  1997/10/09  19:15:42  gray
 * version 0.18.
 *
 * Revision 3.163  1997/10/09  01:30:38  gray
 * version 0.17
 *
 * Revision 3.162  1997/10/08  22:00:15  hightower
 * Ceneter freeplay
 *
 * Revision 3.161  1997/10/08  21:59:47  grjost
 * Fixed remote Join-In button
 *
 * Revision 3.160  1997/10/08  18:51:31  hightower
 * Fixed tourney mode attract mode cycling
 *
 * Revision 3.159  1997/10/07  03:35:08  gray
 * v0.16
 *
 * Revision 3.158  1997/10/06  07:50:13  gray
 * Now doing fog in a sane way.
 *
 * Revision 3.157  1997/10/03  13:52:11  hightower
 * Fix music play bug and allow punch down if no slowpokes
 *
 * Revision 3.156  1997/10/03  04:55:16  hightower
 * Attract mode countdown too fast bug fix
 *
 * Revision 3.155  1997/10/03  00:07:16  grjost
 * Added more rock and atari movies
 *
 * Revision 3.154  1997/10/02  22:13:34  gray
 * Credits display handles up to 12 coin/cedit and up to 99 credits.
 *
 * Revision 3.153  1997/10/02  07:06:35  hightower
 * centered insert coin msg
 *
 * Revision 3.152  1997/10/02  03:59:50  gray
 * version 0.13 - 1st pass final testing.
 *
 * Revision 3.151  1997/10/02  02:04:37  grjost
 * Added old credit and team screens
 *
 * Revision 3.150  1997/10/02  01:25:39  grjost
 * Added and shuffled attract screens
 *
 * Revision 3.149  1997/10/01  22:07:19  gray
 * Reset IRQTIME between attract sequences (and diddle playitagain_time).
 *
 * Revision 3.148  1997/10/01  03:45:42  hightower
 * Changed cdown
 *
 * Revision 3.147  1997/10/01  03:42:46  grjost
...
static void ShowOTeam (BOOL show);
static void ShowJoin (BOOL show);
static void ShowCredits (BOOL show);
static void ShowOCredits (BOOL show);
static void ShowCars (BOOL show);
static void ShowMirror (BOOL show);
...
static BLIT *OCreditsBlit;
static BLIT *MirrorBlit;
static BLIT *CarsBlit;
...
static MULTIBLIT const OTeamMultiBlit[] =
{   /* name      x    y    whtblrfz alpha animFunc id */
    {"TEAMPIC0A", 0,   0,   MUNUSED, 0x00, nil, 0},
    {"TEAMPIC1A", 256, 0,   MUNUSED, 0x00, nil, 1},
    {"TEAMPIC2A", 0,   256, MUNUSED, 0x00, nil, 2},
    {"TEAMPIC3A", 256, 256, MUNUSED, 0x00, nil, 3},
};
...
static MULTIBLIT const MirrorMultiBlit[] =
{   /* name    x    y    whtblrfz alpha animFunc id */
    {"STATIC0_1", 0,   0,   MUNUSED, 0x00, nil, 1},
    {"STATIC0_2", 256, 0,   MUNUSED, 0x00, nil, 1},
    {"STATIC0_3", 0,   256, MUNUSED, 0x00, nil, 1},
    {"STATIC0_4", 256, 256, MUNUSED, 0x00, nil, 1},
};

static MULTIBLIT const CarsMultiBlit[] =
{   /* name    x    y    whtblrfz alpha animFunc id */
    {"STATIC1_1", 0,   0,   MUNUSED, 0x00, nil, 1},
    {"STATIC1_2", 256, 0,   MUNUSED, 0x00, nil, 1},
    {"STATIC1_3", 0,   256, MUNUSED, 0x00, nil, 1},
    {"STATIC1_4", 256, 256, MUNUSED, 0x00, nil, 1},
};
...
static MULTIBLIT const OCreditsMultiBlit[] =
{   /* name    x    y    whtblrfz alpha animFunc id */
    {"CRED00A", 0,   0,   MUNUSED, 0x00, nil, 1},
    {"CRED01A", 256, 0,   MUNUSED, 0x00, nil, 1},
    {"CRED02A", 0,   256, MUNUSED, 0x00, nil, 1},
    {"CRED03A", 256, 256, MUNUSED, 0x00, nil, 1},
};
...
InitAttract (void)
{
    ...
    gAttractTimes[ATR_MOVIE52] = 20;    /*  Rush logo A */
    gAttractTimes[ATR_MOVIE53] = 20;    /*  Rush logo A */
...
ShowAttract (S16 func, BOOL show)
{
    ...
    /* Default to no fog. If fog/tint desired, override in specific Show routine. */
    if(show)
        SetFog(FOG_OFF, 0, 0);
    ...
    switch (func)
    {
        ...
        case ATR_HISCORE7: ShowHiScore (show, 6); break;
        case ATR_CREDITS: ShowCredits (show); break;
        case ATR_OCREDITS: ShowOCredits (show); break;
        case ATR_MIRROR: ShowMirror (show); break;
        case ATR_CARS: ShowCars (show); break;
        case ATR_3DFXLOGO: Show3dfxLogo (show); break;
        case ATR_TEAM: ShowTeam (show); break;
        case ATR_OTEAM: ShowOTeam (show); break;
        case ATR_ATARILOGO:
        case ATR_ATARILOGO2:
        case ATR_ATARILOGO3: ShowMovie (show, 1); break;
        case ATR_MOVIE2: ShowMovie (show, 2); break;
        case ATR_MOVIE3: ShowMovie (show, 3); break;
        case ATR_MOVIE4: ShowMovie (show, 4); break;
        case ATR_MOVIE5:
        case ATR_MOVIE52:
        case ATR_MOVIE53: ShowMovie (show, 5); break;
...
raceAvailable( void )
{
    ...
    {
        if( i != gThisNode &&
            TourneyOn() == TourneyNode( i ) &&
            gn_state[i].alive &&
            gn_state[i].d.state == TRKSEL ) return 1;
    }
...
/*---------------------------------------------------------------------------*/
        ...
        ( raceAvailable() && attractFunc != ATR_JOIN ) )
    {
        ...
        else
        {
            ...
            while (((skip % 5) && (attractFunc == ATR_CREDITS || attractFunc == ATR_3DFXLOGO ||
                   attractFunc == ATR_OTEAM || attractFunc == ATR_OCREDITS || attractFunc == ATR_TEAM)) ||
                   (attractFunc == ATR_MOVIE4 && Random ((F32) IRQTIME) > 900 * ONE_SEC) ||
                   (attractFunc == ATR_ADVERT && TourneyOn ()));
        }

#if 0
        /* reset interrupt timer between attract segments. */
        dtime = IRQTIME - playitagain_time;
        reset_IRQTIME();
        if(playitagain_time)
            playitagain_time = IRQTIME - dtime;
#endif
    ...
    if( attractFunc == ATR_JOIN && gstate == ATTRACT )
    {
        ...
        for (i = 0; i < MAX_LINKS; i++)
        {
            if( TourneyOn() == TourneyNode( i ) &&
                gn_state[i].alive && gn_state[i].d.state == TRKSEL )
            {
                num_in_trk_sel++;

                if( gnGetCountdownTime( i ) > maxtime )
    ...
    if (gStartDuringLoad || chk_start ())
    {
        ...
        */
        switch (attractFunc)
        {
            ...
            case ATR_ATARILOGO:
            case ATR_ATARILOGO2:
            case ATR_ATARILOGO3: AttractMovie (1, false, false);        break;
            ...
            case ATR_MOVIE5:
            case ATR_MOVIE52:
            case ATR_MOVIE53:   AttractMovie (5, false, false);         break;
            default: break;
        }
...
/* ----------------------------------------------------------------------- *
 * Displays or removes the old credits screen.
 * ----------------------------------------------------------------------- */
static void
ShowOCredits (BOOL show)
{
    /* If blit doesn't exist yet, create it. */
    if (!OCreditsBlit && show)
    {
        if (!CheckModelType (MT_ATTRACT))
            return;

        OCreditsBlit =
          NewMultiBlit (0, 0, OCreditsMultiBlit, NBLITS (OCreditsMultiBlit));

        AddJoinNow (OCreditsBlit, 0, true);
    }

    if (!show && OCreditsBlit)
    {
        /*  Remove the screen. */
        RemoveMsgs ();
        RemoveBlit (OCreditsBlit);
        OCreditsBlit = nil;
    }
...
/* ----------------------------------------------------------------------- *
 * Displays or removes the additional cars screen.
 * ----------------------------------------------------------------------- */
static void
ShowCars (BOOL show)
{
    ...
    if (!CarsBlit && show)
    {
        if (!CheckModelType (MT_ATTRACT))
            return;
        CarsBlit = NewMultiBlit (0, 0, CarsMultiBlit, NBLITS (CarsMultiBlit));
        AddJoinNow (CarsBlit, 0, true);
    }
...
/* ----------------------------------------------------------------------- *
 * Displays or removes the mirror screen.
 * ----------------------------------------------------------------------- */
static void
ShowMirror (BOOL show)
{
    /* If blit doesn't exist yet, create it. */
    if (!MirrorBlit && show)
    {
        if (!CheckModelType (MT_ATTRACT))
            return;
        MirrorBlit = NewMultiBlit (0, 0, MirrorMultiBlit, NBLITS (MirrorMultiBlit));
        AddJoinNow (MirrorBlit, 0, true);
    }
...
ShowOTeam (BOOL show)
{
    ...
    if (!OTeamBlit && show)
    {
        if (!CheckModelType (MT_ATTRACT))
            return;
        OTeamBlit = NewMultiBlit (0, 0, OTeamMultiBlit, NBLITS (OTeamMultiBlit));
        AddJoinNow (OTeamBlit, 0, true);
    }
...
AnimateStart (BLIT * blt)
{
    ...
    if (Hidden (blt, attractFunc == ATR_OCREDITS ||
                     attractFunc == ATR_OTEAM ) )
        return 1;
...
AnimateTimer (BLIT * blt)
{
    ...
    BOOL hid = ( (sec   <= 5 && (timer & 0x100)) || timer < (ONE_SEC/4) );
...
AnimateJoin (BLIT * blt)
{
    ...
    if (TourneyOn () && gTourney.ext_start)
    {
        ...
        else return AnimateBlink( blt );
    }

    if( !gJoinInPossible || ((gstate == PLAYGAME || gstate == COUNTDOWN ||
        gstate == PREPLAY2) && !demo_game))
    {
        if( gWait4TrkPlaying )
        {
            gWait4TrkPlaying = 0;
...
AnimateCreds(BLIT *blt)
{
    ...
    {
        case 0:    /*    Label */
            blt->X = SCR_MH - ((freePlay) ? 41 : 89);
            updateBoolean( blt, freePlay, "FREEPLAY", "CREDITS" );
            blt->AnimFunc = nil;
            break;
        case 1:    /* Eight generic digits for credit string. "cc nn/dd" */
            /* do credit string formatting on 1st digit. */
            tcredits = (credits > 99) ? 99 : credits;
            tnumer   = (numer > 99)   ? 99 : numer;
            tdenom   = (denom > 99)   ? 99 : denom;
            if(tcredits == 0)    /* Special format: just show partial credits. */
                sprintf(credstr,"%d/%d      ",tnumer,tdenom);
            else if(tnumer == 0)    /* Special format: just show full credits.  */
                sprintf(credstr,"%d        ",tcredits);
            else            /* Generic format: "cc nn/dd" */
                sprintf(credstr,"%d %d/%d    ",tcredits,tnumer,tdenom);
            /* fall through to blit updates. */
        ...
        case 7:
        case 8:
            /* update blit character. */
            bltchar = credstr[id-1];
            if (Hidden(blt, freePlay || bltchar == ' '))
                return 1;
            return UpdateNumber( blt, (bltchar == '/') ? 10 : (bltchar - '0') );
    }
