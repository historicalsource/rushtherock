 ...
 *
 *	======================================================================
 *	$Author: gray $						$Date: 1997/10/13 07:50:28 $
 *	$Revision: 3.65 $						$Locker:  $
 *	======================================================================
 *	Change Log:
 *
 *	$Log: hud.c,v $
 * Revision 3.65  1997/10/13  07:50:28  gray
 * Don't show bonus time in demo_game.
 *
 * Revision 3.64  1997/10/11  06:10:12  hightower
 * Prevent game crash with nodrones
 *
 * Revision 3.63  1997/10/11  01:43:20  gray
 * Prevent bogus marker positions.
 *
 * Revision 3.62  1997/10/10  23:52:13  hightower
...
static F32		marker_pos[MAX_LINKS][3];
...
static int AnimateMarker(BLIT *blt) 
{
	...
	offscreen = (	slot == -1 ||
			(marker_pos[slot][0] < -32.0) ||
			(marker_pos[slot][0] > 544.0) ||
			(marker_pos[slot][1] < -32.0) ||
			(marker_pos[slot][1] > 416.0) ||
...
static int AnimateCTimer(BLIT *blt) 
{
	...
	if (Hidden(blt, sec <= 5 && (timer & 0x100)))
		return 1;
...
static int AnimateBonus(BLIT *blt) 
{
	...
	BOOL hide = remain < 0 || (GetCountdownTime() & 0x80) || demo_game;
