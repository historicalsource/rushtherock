 ...
 *
 *	======================================================================
 *	$Author: gray $						$Date: 1997/09/02 09:03:31 $
 *	$Revision: 3.34 $						$Locker:  $
 *	======================================================================
 *	Change Log:
 *
 *	$Log: camera.c,v $
...
void View3Cam(F32 pos[3], F32 uvs[3][3])
{
	...
	CAR_DATA *car = &game_car[this_node];
	S32	i,tcnt;
	F32	susavg, scale, magvel, rad, len;
	U32	dtime;
...
if(lasttime == 0)
 CopyVector(uvector, flyup);
	...
	if(susavg > 0.5)
		{
		fudge_out = susavg - 0.5;
		for(i=0;i<3;i++)
			pos[i] += (car->dr_uvs[i][1] * fudge_out);
		}
	...
	if(fu == 0.0)
		...
		{
		for(i=0;i<3;i++)
			flyup[i] = car->dr_uvs[i][1];
		}
	else
		{
		flyup[0] = car->dr_uvs[0][1] * fu;
		flyup[1] = car->dr_uvs[1][1] + ((1.0 - car->dr_uvs[1][1]) * fu);
		flyup[2] = car->dr_uvs[2][1] * fu;
		len = magnitude(flyup);
		if(len < 0.001)
			CopyVector(uvector, flyup);
		else
			ScaleVector(flyup, 1.0/len, flyup);
		}



	/* get behind-the-car position.. */
	for(i=0;i<3;i++)
	...
	if(f < 1.0)
		...
		if(magvel < 1.0)
			...
			ScaleAddVector(flyup, up_dist, flyvec, flyvec);
			direction(flyvec, flyvec);
			ScaleVector(flyvec, rad, flyvec);
			}
		}

	/* copy or interpolate cam pos. */
	if(f == 0.0)
