 ...
 *
 *	======================================================================
 *	$Author: grjost $						$Date: 1997/09/17 19:40:58 $
 *	$Revision: 3.111 $						$Locker:  $
 *	======================================================================
 *	Change Log:
 *
 *	$Log: select.c,v $
...
#define CAR_DELAY_TIME	(ONE_SEC/3)				/* Delay before car lock can occur */
...
static BlitFunc AnimateTrans, AnimateFeedback, AnimateBar, AnimateSolo, 
				AnimateCar, AnimateCarSel, AnimateTrack, 
				AnimateWait, AnimateWaitJ, AnimateDanger, AnimateTrkNum,
				AnimatePanel,
#if KETCHUP
				AnimateCatchup,
#endif
...
static S32		tlock_sound_delay, twait_delay;
...
const S16  car_type_sound[4] = { S_BEEPWARN1, S_BEEPWARN2, S_BEEPWARN3, S_KLAXON };
const S16  kill_car_type_sound[4] = { S_KBEEPWARN1, S_KBEEPWARN2, S_KBEEPWARN3, S_KKLAXON };
const S16  track_picked_sound[8] = {	S_YOU1, S_YOU2, S_YOU3, S_YOUROCK,
					S_YOU5, S_YOU6, S_YOU7, S_YOU8 };
const S16  waiting_on_sound[8] = {	S_WAIT1, S_WAIT2, S_WAIT3, S_WAITROCK,
					S_WAIT5, S_WAIT6, S_WAIT7, S_WAIT8 };
const S16  others_locked_sound[7] = { S_2CARS, S_3CARS, S_4CARS, S_5CARS, S_6CARS, S_7CARS, S_8CARS };
const S16  kill_others_locked_sound[7] = { S_K2CARS, S_K3CARS, S_K4CARS, S_K5CARS, S_K6CARS, S_K7CARS, S_K8CARS };
...
{	/* name			 		            x         y whtblrfz alpha animFunc        id */
	...
	{"TRKPIC01",                      152,       12, MUNUSED, 0x00,AnimateTrack,   -1},
...
{
	static S32 lastSend;
	static S16 link_state;
	U8	old_flag = solo_flag;
	...
	if (oldTrack == -1)
		...
		old_flag = solo_flag = (levels & SW_REVERSE) != 0;
	...
	if ((IRQTIME - gBeginTime > TRK_DELAY_TIME) &&
		( TourneyOn() ||
		  (GasPressed(true) || (edges & SW_ABORT) || (GetCountdownTime() < ONE_SEC))))
		{
		if (TourneyOn())
			{
			ShowTrackSelect(true);
...
/* hack to allow solo fall through without everyone falling through. */
			...
			if (link_state == 0 && gTrackLocked == false)
				SOUND (S_TRANSELECT);
			}
...
send:
	...
	if(edges & SW_VIEW1)
		gMirror = (gMirror) ? 0 : 1;
	if(gMirror)
		DBG_printf(29,10,YEL_PALB,"MIRROR");
	else
		DBG_printf(29,10,YEL_PAL,"     ");
...
void waiting_for_others (S16 type)
{
	...
	/* now see if we need to tell others of potential single link going */
	if (IRQTIME > twait_delay)
		{
		wait_track = -1;
		for (i=0; i<MAX_LINKS; i++)
			{
			if (i != gThisNode &&
				( (type == 0 && (gLink[i].state==LOCKED || gLink[i].state==JOINING)) ||
				  (type == 1 && (gLink[i].flags & LINK_TLOCK))))
				{
				if (type == 0)
					wait_track = 0;
				else if (wait_track == -1)
					wait_track = gLink[i].track;
				else if (wait_track != gLink[i].track)
					{
					wait_track = -1;
					break;
					}
...
static S16 do_link_sound (S16 link_state)
{
	...
	/* count number of players other than yourself that have locked on your track */
	for (i=0,count=0; i<MAX_LINKS; i++)
		{
		if (gLink[i].track == trackno   &&
			gLink[i].flags & LINK_TLOCK &&
			i != gThisNode)
			count++;
		}

	if (count == 7 && link_state != 2)
		{
		SOUND (S_8WAY);
		link_state = 2;
		}
	else if (count != 0 && link_state == 0)
		{
		SOUND (S_LINKED);
		link_state = 1;
		}
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateCarSel(BLIT *blt)
{
    U16 		i,j, objnum;
	S32			index;
	...
	char 		name[20];
    F32			deg, mat[3][3], rpy[3], pos[3] = {0,0,0};
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateTrkNum(BLIT *blt)
{
	S32 bltnum = trackno;
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateSWheel(BLIT *blt)
{
	char str[20];
    F32	mat[3][3], 
		rpy[3] = {0,0,0};
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateSolo(BLIT *blt)
{
	Hidden(blt, !((gLink[gThisNode].flags & LINK_SOLO) || 
				  ((levels & SW_REVERSE) && !TourneyOn())));
	return 1;
}	/* end AnimateSolo */
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateDrone(BLIT *blt)
{
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateTrans(BLIT *blt)
{
	static F32 vel;
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateFeedback(BLIT *blt)
{
 ...
 * Called every frame to update the Drones/No Drones button.
 * ----------------------------------------------------------------------------- */
static int AnimateCatchup(BLIT *blt)
{
 ...
 * ----------------------------------------------------------------------------- */
static int	AnimateWait(BLIT *blt)
{
 ...
 * Called every frame to update the "Wait for others to select" blit.
 * ----------------------------------------------------------------------------- */
static int	AnimateWaitJ(BLIT *blt)
{
	BOOL	hide = (blt->AnimID == 0 && !gTrackLocked) ||
				   (blt->AnimID == 1 && !gClutchLocked);
 ...
 * ----------------------------------------------------------------------------- */
static int	AnimateDanger(BLIT *blt)
{
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateLink(BLIT *blt)
{
	S16 		i;
	U8			track;
	LinkInfo	*p;
 ...
 * ----------------------------------------------------------------------------- */
int	AnimateWheel(BLIT *blt)
{
	S16	step = (IRQTIME >> 7) % 10;
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateTrack(BLIT *blt)
{
	S32 index;
	char trkpic[16];

	if (language == LANG_JAPANESE)
		sprintf( trkpic, "JTRKPIC0%d", trackno );
	else
		sprintf( trkpic, "TRKPIC0%d", trackno );
 ...
 * ----------------------------------------------------------------------------- */
static int AnimateCar(BLIT *blt)
{
	...
	return updateCar( blt, slot, ((p->flags & LINK_BLINK) && (IRQTIME & 0x100)) ? 1:0 );
 ...
 * data is 
 * ----------------------------------------------------------------------------- */
static int AnimateBar(BLIT *blt)
{
	U8			bar		= (blt->AnimID >> 4) & 0xF;
	U8			segment	= (blt->AnimID & 0xF);
	S16			t, car;
	static const S16	carSpeed[]		= { 100, 200, 350, 500 };
	static const S16	carControl[]	= { 500, 350, 200, 100 };
 ...
 * ----------------------------------------------------------------------------- */
int updateCar( BLIT *blt, S8 index, S8 dark )
{	
	blt->Hide 		= false;
 ...
 * ----------------------------------------------------------------------------- */
static int	AnimateGlow(BLIT *blt)
{
	S16	col, slot, trk;

	blt->Alpha = (gTrackLocked ||(IRQTIME & 0x100)) ? 0xFF : 0x00;
 ...
 * ----------------------------------------------------------------------------- */
int updateBoolean( BLIT *blt, BOOL val, char *t, char *f )
{
	if (val != blt->AnimDTA) 
		{
		blt->AnimDTA = val;
...
/* ----------------------------------------------------------------------------- */
void ShowCarSelect( BOOL active )
{
...
static void ClutchSel(void)
{
	...
	/*	Handle tourney mode overrides. */
	tourney_lock = false;
	if (TourneyOn() && gTourney.tranny >= 0 && (IRQTIME - gBeginTime > CAR_DELAY_TIME))
		{
...
{
	return (S32)play_time - (S32)GetElapsedTime();
 ...
 * Returns true if gas has been pressed.
 * ----------------------------------------------------------------------------- */
void SetGasReleased(BOOL released)
{
 ...
 * ----------------------------------------------------------------------------- */
void BlinkUnlessPressed( U32 light, U32 button )
{
	U32		cur = ctl_mod_latch(0);
	BOOL 	on	= (levels & button) || (IRQTIME & BLINK_TIME);
