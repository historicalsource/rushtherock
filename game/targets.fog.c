 ...
 *
 *	======================================================================
 *	$Author: grjost $						$Date: 1997/09/30 02:20:38 $
 *	$Revision: 3.70 $						$Locker: gray $
 *	======================================================================
 *	Change Log:
 *
 *	$Log: targets.c,v $
...
#define FOG_TINT      2
#define FOG_REAL      3
#define FOG_TINT_REAL 4

static S32 fogMode, last_fogMode;
...
void SetFog(S32 mode, U32 density, U32 color)
{
	...
	/* sanity check data */
	if((mode < FOG_OFF) || (mode > FOG_TINT_REAL))
		mode = FOG_OFF;
...
void FogReal(void)
{
	static F32 last_mindist, last_maxdist, last_color, last_maxfog;

	if((last_fogMode == FOG_REAL) &&
	   (last_mindist == dbgFogMinDist[trackno]) &&
	   (last_maxdist == dbgFogMaxDist[trackno]) &&
	   (last_color == dbgFogColor[trackno]) &&
	   (last_maxfog == dbgFogMaxLevel[trackno]))
		return;

	MakeFogRamp(dbgFogColor[trackno],
		dbgFogMinDist[trackno],dbgFogMaxDist[trackno],
		0,dbgFogMaxLevel[trackno]);
...
void MakeFogRamp(U32 color, F32 mindist, F32 maxdist, F32 minfog, F32 maxfog)
{
	...
	for(i=0;i<GR_FOG_TABLE_SIZE;i++)
		...
		if(tabdist <= mindist)
			...
			fogTable[i] = (GrFog_t)(delfog * ((tabdist - mindist)/rampdist));
		}
	fogTable[0] = (GrFog_t)0;
...
/*DBG_printf(20,4,YEL_PALB,"gFogGroup:%08lX",(U32)gFogGroup);*/
	...
	if( !demo_game && !g &&
		...
		{
		trk = trackno;
		mindist = dbgFogMinDist[trk];
		maxdist = dbgFogMaxDist[trk];
		color = dbgFogColor[trk];
		density = dbgFogMaxLevel[trk];
		StaticFog( mindist, maxdist, density, color );
		return;
		}
	ZoidBGColor = 0;
#endif
	...
	if (demo_game || !g || (dlevels & SW_DEBUG15))
		return;
...
#endif
EMSG("FogTable color:0x%08lX",color);
	grFogColorValue(color);
	grFogTable(fogTable);
	grFogMode(GR_FOG_WITH_TABLE);
}	/* end MakeFogTable */
...
{
	S32 i;
	F32 fog,rampdist,tabdist;
	static F32 last_mindist,last_maxdist,last_maxfog;

	if((mindist==last_mindist)&&(maxdist==last_maxdist)&&(maxfog==last_maxfog)&&(color==lastColor))
		return;
	...
	for(i=0;i<GR_FOG_TABLE_SIZE;i++)
		...
		tabdist = guFogTableIndexToW(i);
		if(tabdist <= mindist)
			fogTable[i] = (GrFog_t)0;
		else if(tabdist >= maxdist)
			fogTable[i] = (GrFog_t)maxfog;
		else	/* in fog ramp */
			fogTable[i] = (GrFog_t)(maxfog * ((tabdist - mindist)/rampdist));
		}

EMSG("StaticFog color:0x%08lX",color);
	ZoidBGColor = color;
	grFogColorValue( color );
	grFogTable(fogTable);
	grFogMode(GR_FOG_WITH_TABLE);
...
#if 1
	if(density == 0)
		SetFog(FOG_REAL, density, color);
/*	else if(demo_game)
		SetFog(FOG_TINT_REAL, density, color);*/
	else
		SetFog(FOG_TINT, density, color);
	return;
#endif
