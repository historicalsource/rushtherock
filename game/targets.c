 ...
 *
 *	======================================================================
 *	$Author: gray $						$Date: 1997/10/14 22:27:05 $
 *	$Revision: 3.73 $						$Locker:  $
 *	======================================================================
 *	Change Log:
 *
 *	$Log: targets.c,v $
 * Revision 3.73  1997/10/14  22:27:05  gray
 * Go back to using double target fog radius so T3 fog works.
 *
 * Revision 3.72  1997/10/13  03:57:32  gray
...
/* Fog for game and non-darkened attract demo. */
void FogTrack(void)
{
	static S32 last_tgtfog;
	S32 tgtfog,force;

	/* NOTE: Target Fog will not work well on tracks */
	/* that have standard fog. The fog will snap to  */
	/* the target fog when within the target radius. */
	/* A track should use either normal fog OR target fog. */

	tgtfog = gFogGroup && !demo_game;
	force = ((last_tgtfog != tgtfog) ||
		 (last_fogMode != FOG_TRACK));
	last_tgtfog = tgtfog;
...
/* Pocket fog - density based on distance from target center. */
void FogTrackTarget(S32 force)
{
	static F32 last_density;
	static S32 last_clip;
	static U32 last_color;
	F32		density, dist, f;
	U32		color, max;
	S32		clip;
	Target	*t, *fogT;
