 ...
 *
 *  ======================================================================
 *  $Author: gray $                       $Date: 1997/09/26 09:39:30 $
 *  $Revision: 3.43 $                       $Locker:  $
 *  ======================================================================
 *  Change Log:
 *
 *  $Log: pdu.c,v $
 * Revision 3.43  1997/09/26  09:39:30  gray
 * modify entity state data if mirror mode.
 *
...
PutEntityStatePDU (U16 entID)
{
    ...
    if (!(entID & 0x8000))
    {
        ...
        if(gMirrorMode) /* If we are mirrored, unmirror and send. */
            {
            /* mirror world vectors. */
            p.velocity[1] = -p.velocity[1];
            p.location[1] = -p.location[1];
            p.accel[1] = -p.accel[1];
            p.angularV[0] = -p.angularV[0];
            p.angularV[2] = -p.angularV[2];
            p.quat[0] = -p.quat[0];
            p.quat[2] = -p.quat[2];
            p.steerangle = -p.steerangle;
            /* swap car left for right... */
            temp = p.shadow_h[0]; p.shadow_h[0] = p.shadow_h[1]; p.shadow_h[1] = temp;
            temp = p.shadow_h[2]; p.shadow_h[2] = p.shadow_h[3]; p.shadow_h[3] = temp;
            temp = p.shadow_v[0]; p.shadow_v[0] = p.shadow_v[1]; p.shadow_v[1] = temp;
            temp = p.shadow_v[2]; p.shadow_v[2] = p.shadow_v[3]; p.shadow_v[3] = temp;
            temp = p.tireV[0]; p.tireV[0] = p.tireV[1]; p.tireV[1] = temp;
            temp = p.tireV[2]; p.tireV[2] = p.tireV[3]; p.tireV[3] = temp;
            temp = p.suscomp[0]; p.suscomp[0] = p.suscomp[1]; p.suscomp[1] = temp;
            temp = p.suscomp[2]; p.suscomp[2] = p.suscomp[3]; p.suscomp[3] = temp;
            bits = p.appearance & ~App_SHBITS;
            bits |= ((p.appearance & App_SR2BITS) >> 2);
            bits |= ((p.appearance & App_SL2BITS) << 2);
            bits |= ((p.appearance & App_SR1BITS) >> 1);
            bits |= ((p.appearance & App_SL1BITS) << 1);
            p.appearance = bits;
            bits = p.look & ~(ROADCODE_MASK << ROADCODE_OFFSET);
            bits |= ((p.look & (Look_LF_ROADCODE|Look_LR_ROADCODE)) >> 3);
            bits |= ((p.look & (Look_RF_ROADCODE|Look_RR_ROADCODE)) << 3);
            p.look = bits;
            }
    }
...
/*  RTEST( "timeStamp",     d->timeStamp    ); */
    ...
    if(gMirrorMode) /* If we are mirrored, mirror info from net. */
        {
        /* mirror world vectors. */
        d->vel[1] = -d->vel[1];
        d->pos[1] = -d->pos[1];
        ...
        d->quat[0] = -d->quat[0];
        d->quat[2] = -d->quat[2];
        d->steerangle = -d->steerangle;
        /* swap car left for right... */
        temp = d->suscomp[0]; d->suscomp[0] = d->suscomp[1]; d->suscomp[1] = temp;
        temp = d->suscomp[2]; d->suscomp[2] = d->suscomp[3]; d->suscomp[3] = temp;
        temp = d->tireV[0]; d->tireV[0] = d->tireV[1]; d->tireV[1] = temp;
        temp = d->tireV[2]; d->tireV[2] = d->tireV[3]; d->tireV[3] = temp;
        temp = d->shadow_h[0]; d->shadow_h[0] = d->shadow_h[1]; d->shadow_h[1] = temp;
        temp = d->shadow_h[2]; d->shadow_h[2] = d->shadow_h[3]; d->shadow_h[3] = temp;
        temp = d->shadow_v[0]; d->shadow_v[0] = d->shadow_v[1]; d->shadow_v[1] = temp;
        temp = d->shadow_v[2]; d->shadow_v[2] = d->shadow_v[3]; d->shadow_v[3] = temp;
        bits = d->appearance & ~App_SHBITS;
        bits |= ((d->appearance & App_SR2BITS) >> 2);
        bits |= ((d->appearance & App_SL2BITS) << 2);
        bits |= ((d->appearance & App_SR1BITS) >> 1);
        bits |= ((d->appearance & App_SL1BITS) << 1);
        d->appearance = bits;
        bits = d->look & ~(ROADCODE_MASK << ROADCODE_OFFSET);
        bits |= ((d->look & (Look_LF_ROADCODE|Look_LR_ROADCODE)) >> 3);
        bits |= ((d->look & (Look_RF_ROADCODE|Look_RR_ROADCODE)) << 3);
        d->look = bits;
        }
